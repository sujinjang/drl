# Actor-Critic with (1) value function as a baseline and (2) reward-to-go as a reward summation
...
def actor_critic(sess, exp, pg_model, value_model, env, gamma, isRTG=True, n_iterations=100, n_batch=100,
    ...
    for iter in range(n_iterations):
        ...
        while True:
            ...
            for step in range(len(episode_states)):
                target = episode_rewards[step] + gamma*ValueEstimator.predict(episode_next_states[step], sess=sess)
                advantage = target - ValueEstimator.predict(episode_states[step], sess=sess)
        ...
        batch_targets = np.concatenate([traj["target"] for traj in trajectories])
        batch_advantages = np.concatenate([traj["advantage"] for traj in trajectories])

        # Update value estimator
        value_model.update(states=batch_states, targets=batch_targets, sess=sess)
        ...
        advantages = [traj["advantage"].sum() for traj in trajectories] # TODO: DOESN'T LOOK NECESSARY OR INFORMATIVE
...
if __name__ == "__main__":
    ...
    for exp in range(args.n_experiments):
        ...
        with tf.Session() as sess:
            ...
            actor_critic(sess=sess, exp=exp, pg_model=pg_model, value_model=value_model, env=env, gamma=args.gamma,
